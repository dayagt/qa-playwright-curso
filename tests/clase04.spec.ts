...
test.describe('Clase 04 - Flujo completo de usuario en DemoBlaze', () => {
  ...
  await page.locator('#name').fill('Lourdes');
  await page.locator('#country').fill('Guatemala');
  await page.locator('#city').fill('Santa Rosa');
...
test('Reto 3 - Usar clear() en un campo', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('link', { name: 'Log in', exact: true }).click();
  await page.waitForSelector('#logInModal', { state: 'visible' });

  const campoUsuario = page.locator('#loginusername');
  await campoUsuario.fill('texto temporal');
  await campoUsuario.clear();

  
  const valor = await campoUsuario.inputValue();
  expect(valor).toBe('');
});
