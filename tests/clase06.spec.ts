...
test('Verificar que el inventario tiene 6 productos',
async ({ page }) => {
const loginPage = new LoginPage(page);
const inventoryPage = new InventoryPage(page);
await loginPage.navigate();
...
test('Ordenar productos de mayor a menor precio',
async ({ page }) => {
const loginPage = new LoginPage(page);
const inventoryPage = new InventoryPage(page);
await loginPage.navigate();
...
test('Reto 1 - Completar compra con CheckoutPage', async ({ page }) => {
  const loginPage = new LoginPage(page);
  const inventoryPage = new InventoryPage(page);
  const cartPage = new CartPage(page);
  ...
  await checkoutPage.fillForm('Dayanna', 'Lopez', '12345');
  await checkoutPage.finishPurchase();
});

test('Reto 2 - Logout con MenuPage', async ({ page }) => {
  const loginPage = new LoginPage(page);
...
test('Reto 3 - Quitar producto y badge desaparece', async ({ page }) => {
  const loginPage = new LoginPage(page);
  const inventoryPage = new InventoryPage(page);
  ...
  // Validar que el badge ya no está visible
